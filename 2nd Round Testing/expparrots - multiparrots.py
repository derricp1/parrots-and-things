...
def PARROTS(t, v, parrotee_percent, parroter_percent):
    ...
    spcp = 0 # 1 = spcp = synch pseudo change protocoal, 0 = aosa = anon online svc access protocol svc access protocol
    ...
    for ti in range(1): # initialize vehicles at random coordinates on road grid
        for vi in range(v):
            ...
            parroted_id_2.append(-1) # no parroting at initialization
            p_anonymity.append(0) # at first there are zero parroters from previous groups (PAS)
            p_anonymity_2.append(0) # at first there are zero parroters from previous groups (PAS)
            ...
            p_timeout_2.append(-1) # none because parroting has not started yet
    ...
    for ti in range(t): # move vehicles to nearby coordinates on road grid
        ...
        for vi in range(v):
            ...
            if p_timeout_2[vi] != -1:
                p_timeout_2[vi] -= 1
                if p_timeout_2[vi] == 0: 
                    p_timeout_2[vi] = -1 # end parroting
                    parroted_id_2[vi] = -1
       ...
       # -------------------------------------------------------------------- #
        ...
        for vi in range(v):
            ...
            if parroted_id[vi] == -1:
                for pi in range (v): # search all vehicles
                    if leader[pi] == leader[vi]:       # find vehicle in same group
                        if pi != vi and pi != parroted_id_2[vi]: # but not self same vehicle
            ...
            if parroted_id_2[vi] == -1:
                for pi in range (v): # search all vehicles
                    if leader[pi] == leader[vi]:       # find vehicle in same group
                        if pi != vi and pi != parroted_id[vi]: # but not self same vehicle
                            if parrotee[pi] == 1:      # pi wants a parrot
                                if parroter[vi] == 1:  # vi wants to be a parrot
                                    parroted_id_2[vi] = pi  # vi parrots for pi
                                    p_timeout_2[vi] = v_timeout[parroted_id_2[vi]]
                                    # parroting times out when parrotee's pseudoid times out
                                    p_anonymity_2[parroted_id_2[vi]] += 1 #gpc
                                    parrot_counter += 1
                                    break     # get out of "for" loop
            ...
            for pi in range(v):
                ...
                if parroted_id_2[pi] > -1:
                    p_anonymity_2[parroted_id[pi]] += anonymity[pi]
        ...
        # -------------------------------------------------------------------- #
            ...
            entropy = entropy + math.log(anonymity[vi]+p_anonymity[vi]+p_anonymity_2[vi],2) # see eq p.101
            if anonymity[vi] + p_anonymity[vi] + p_anonymity_2[vi] == 1: # as1 ia number of cars with as=1
               as1 = as1 + 1
            ...
            cum_p_anonymity += p_anonymity_2[vi]
            if parroted_id[vi] > -1:
                parrot_counter += 1
...
#   Main
# ----------------------------------------------------------------------------- #
print (PARROTS(2000,400,50,50))
print ("###############################################")
print (PARROTS(2000,800,50,50))
print ("###############################################")
print (PARROTS(2000,1200,50,50))
print ("###############################################")
print (PARROTS(2000,1600,50,50))
print ("###############################################")
print (PARROTS(2000,2000,50,50))
print ("###############################################")
print (PARROTS(2000,2400,50,50))
print ("###############################################")
print (PARROTS(2000,2800,50,50))
print ("###############################################")
print (PARROTS(2000,3200,50,50))
print ("###############################################")
print (PARROTS(2000,3600,50,50))
print ("###############################################")
print (PARROTS(2000,4000,50,50))
